...
import { GetLearnerDetail } from "../../graphql/queries/authentication.query.js";
import { ActivateLearner } from "../../graphql/mutations/authentication.mutation.js";
...
function VerifyIssuer() {
  ...
  const { loading, error, data } = useQuery(GetLearnerDetail, {
    variables: { learnerId: query.payload },
  });
  const [
    activateMOEMutation,
    {
  ...
  useEffect(() => {
    if (data) {
      setQrCodeState(data.GetLearnerDetail.qrCode);
    }
  }, [data]);
    ...
    activateMOEMutation({
      ...
      onCompleted: (res) => {
        ...
        setTimeout(() => {
          ...
          push(`/learner/set-password?email=${data.GetLearnerDetail.email}`);
        }, 500);
      },
      onError: (errors) => {
